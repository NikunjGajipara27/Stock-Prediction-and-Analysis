...
def gyaani_baba_prediction(symbol, days=120):
    ...
    try:
        ...
        # Drop rows with missing values after creating indicatorss
        data = data.dropna()
        
        X = data[['Open', 'High', 'Low', 'Close', 'Volume', 'SMA_50', 'SMA_200', 'RSI', 'MACD']]
        y = data['Close'].shift(-1).dropna().values.ravel()  # Ensure it's 1D
        # Drop rows with NaN values
        data = data.dropna()

        # Align X and y
        X = X.iloc[:-1]  # Drop last row to match y length
        
        # Make sure `X` is still a DataFrame (2D)
        ...
        y_test = y_test.ravel()    # Convert to 1D
        ...
        last_data = X.iloc[-1].values.flatten().reshape(1, -1)  # Ensure it's (1, features)
        print("Initial last_data shape:", last_data.shape)  # Debugging print
        # print("X shape:", X.shape)  # Should be (n_samples, n_features)
        # print("last_data shape before predict:", last_data.shape)  # Should be (1, n_features)

        predictions = []
        for i in range(days):
            print("🚀 Debugging Shapes Before Prediction 🚀")
            print("X shape:", X.shape)  # Should be (rows, features)
            print("y shape:", y.shape)  # Should be (rows,) (MUST BE 1D)
            print("last_data shape before predict:", last_data.shape)  # Should be (1, features)
            print("y_train shape:", y_train.shape)  # Should be (rows,) (1D)
            print("y_test shape:", y_test.shape)  # Should be (rows,) (1D)

            # Now predict
            pred = model.predict(last_data.squeeze().reshape(1, -1))[0]
            predictions.append(pred)
            last_data = np.roll(last_data, -1).reshape(1, -1)  # Ensure it stays (1, features)
            # last_data = last_data.reshape(1, -1)  # Keep shape as (1, features)
            last_data[0, -1] = pred  # Update last value
            print(f"Iteration {_+1}: last_data shape -> {last_data.shape}")  # Debugging print
